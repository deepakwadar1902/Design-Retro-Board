...
export default class Boards extends LightningElement {
    ...
    // Handle the section name input change event
    handleSectionNameChange(event) {
        const sectionId = event.target.dataset.id;
        const sectionName = event.target.value;
        const sectionIndex = this.sections.findIndex((section) => section.id === parseInt(sectionId, 10));

        if (sectionIndex > -1) {
            this.sections[sectionIndex].sectionName = sectionName;
        }
    }
